...
export type LicenseCheckResult =
  | { ok: true; status: 'active' | 'revoked' | 'unknown'; message: string }
...
export function createInviteRecord(input: CreateInviteInput): InviteRecord {
  ...
  return {
    ...
    licenseDays,
    expiresAt: input.expiresAt ?? isoDate(addDays(now, licenseDays)),
    status: 'active',
    createdAt: now.toISOString(),
    features: input.features ?? ['ticketing', 'training', 'desktop'],
    activations: [],
  };
}
...
export async function loadInviteDatabase(dbPath: string): Promise<InviteDatabase> {
  try {
    ...
    return {
      schemaVersion: 1,
      invites: Array.isArray(parsed.invites) ? parsed.invites as InviteRecord[] : [],
...
export async function checkLicense(dbPath: string, request: { licenseId?: string; deviceHash?: string }): Promise<LicenseCheckResult> {
  ...
  const activation = database.invites.flatMap((invite) => invite.activations).find((item) => item.licenseId === request.licenseId && item.deviceHash === request.deviceHash);
  return activation
    ? { ok: true, status: 'active', message: '授权有效' }
    : { ok: true, status: 'unknown', message: '授权记录不存在' };
}
